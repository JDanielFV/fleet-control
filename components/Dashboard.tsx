              ...
              {ctx.activeTab === "dashboard" && (
                ctx.isLoading ? <DashboardSkeleton /> : (
                  <div className="flex-1 flex flex-col overflow-hidden space-y-6 h-full">
                    {/* Hero */}
                    <motion.div custom={0} initial="hidden" animate="visible" variants={tileVariants} className="flex items-center justify-between gap-4 pt-2 px-1 shrink-0">
                      <div>
                        <h1 className="text-[26px] font-black tracking-tight leading-none text-foreground">
                          Buenos días.{ctx.currentTime && <span className="text-muted-foreground ml-2 text-lg font-medium">{ctx.currentTime}</span>}
                        </h1>
                        <p className="text-xs text-muted-foreground mt-2">{ctx.stats.vehicles} vehículos · {ctx.stats.assigned} activos · {ctx.alerts.length} {ctx.alerts.length === 1 ? "alerta" : "alertas"} pendientes</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button onClick={ctx.exportChecklistCsv} className="p-2.5 rounded-full bg-secondary hover:bg-secondary/80 text-foreground transition-all cursor-pointer active:scale-95 shadow-2xs border-none shrink-0" aria-label="Exportar checklists a CSV" title="Exportar CSV"><Download className="w-4 h-4" /></button>
                        <button onClick={() => ctx.setIsBuzonOpen(true)} className="md:hidden relative p-2.5 rounded-full bg-secondary hover:bg-secondary/80 text-foreground transition-all cursor-pointer active:scale-95 shadow-2xs border-none shrink-0" aria-label={`Abrir buzón de alertas. ${ctx.alerts.length} alertas activas`}>
                          <Bell className="w-4 h-4" />
                          {ctx.alerts.length > 0 && <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 bg-red-500 text-white text-[11px] font-black rounded-full flex items-center justify-center">{ctx.alerts.length}</span>}
                        </button>
                      </div>
                    </motion.div>
                    ...
                    <div className="bg-[#ECECEC] rounded-2xl h-12 px-4 flex items-center gap-2 w-full shrink-0 shadow-inner border border-border/40 focus-within:ring-4 focus-within:ring-primary/20 transition-all">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 pl-2 border-l border-border/40">
                        <button onClick={() => { ctx.setAutoOpenDriver(true); ctx.handleTabChange("drivers"); }} className="inline-flex items-center gap-1 px-3 h-8 rounded-lg bg-primary text-white text-[11px] font-bold hover:bg-primary/90 transition-all cursor-pointer active:scale-95 shadow-sm border-none">
                          <User className="w-3.5 h-3.5" /> Chofer
                        </button>
                        <button onClick={() => { ctx.setAutoOpenVehicle(true); ctx.handleTabChange("vehicles"); }} className="inline-flex items-center gap-1 px-3 h-8 rounded-lg bg-primary text-white text-[11px] font-bold hover:bg-primary/90 transition-all cursor-pointer active:scale-95 shadow-sm border-none">
                          <Car className="w-3.5 h-3.5" /> Auto
                        </button>
                        <button onClick={() => ctx.setAssignmentDialogOpen(true)} className="inline-flex items-center gap-1 px-3 h-8 rounded-lg bg-emerald-500 text-white text-[11px] font-bold hover:bg-emerald-600 transition-all cursor-pointer active:scale-95 shadow-sm border-none">
