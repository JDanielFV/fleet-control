...
import DriversSlice from "@/features/drivers/components/DriversSlice";
import VehiclesSlice from "@/features/vehicles/components/VehiclesSlice";
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden space-y-6 h-full">
                    ...
                    <motion.div custom={0} initial="hidden" animate="visible" variants={tileVariants} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4 pt-2 px-1 shrink-0">
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-auto">
                        <button onClick={ctx.exportChecklistCsv} className="p-3.5 md:p-2.5 rounded-full bg-secondary hover:bg-secondary/80 text-foreground transition-all cursor-pointer active:scale-95 shadow-2xs border-none shrink-0" aria-label="Exportar checklists a CSV" title="Exportar CSV"><Download className="w-4 h-4" /></button>
                        <button onClick={() => ctx.setIsBuzonOpen(true)} className="md:hidden relative p-3.5 md:p-2.5 rounded-full bg-secondary hover:bg-secondary/80 text-foreground transition-all cursor-pointer active:scale-95 shadow-2xs border-none shrink-0" aria-label={`Abrir buzón de alertas. ${ctx.alerts.length} alertas activas`}>
                          <Bell className="w-4 h-4" />
                          {ctx.alerts.length > 0 && <span className="absolute -top-1 -right-1 min-w-[14px] md:min-w-[16px] h-3 md:h-4 px-1 bg-red-500 text-white text-[10px] md:text-[11px] font-black rounded-full flex items-center justify-center">{ctx.alerts.length}</span>}
                        </button>
                      </div>
                    </motion.div>
                    ...
                    <div className="bg-[#ECECEC] rounded-2xl min-h-12 px-3 sm:px-4 flex items-center gap-2 w-full shrink-0 shadow-inner border border-border/40 focus-within:ring-4 focus-within:ring-primary/20 transition-all flex-wrap sm:flex-nowrap py-2 sm:py-0">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 pl-0 sm:pl-2 border-l-0 sm:border-l border-border/40 w-full sm:w-auto justify-end">
                        <button onClick={() => { ctx.setAutoOpenDriver(true); ctx.handleTabChange("drivers"); }} className="inline-flex items-center gap-1 px-3 h-11 sm:px-3 md:h-8 rounded-lg bg-primary text-white text-[10px] md:text-[11px] font-bold hover:bg-primary/90 transition-all cursor-pointer active:scale-95 shadow-sm border-none">
                          <User className="w-4 h-4 md:w-3.5 md:h-3.5" /> Chofer
                        </button>
                        <button onClick={() => { ctx.setAutoOpenVehicle(true); ctx.handleTabChange("vehicles"); }} className="inline-flex items-center gap-1 px-3 h-11 sm:px-3 md:h-8 rounded-lg bg-primary text-white text-[10px] md:text-[11px] font-bold hover:bg-primary/90 transition-all cursor-pointer active:scale-95 shadow-sm border-none">
                          <Car className="w-4 h-4 md:w-3.5 md:h-3.5" /> Auto
                        </button>
                        <button onClick={() => ctx.setAssignmentDialogOpen(true)} className="inline-flex items-center gap-1 px-3 h-11 sm:px-3 md:h-8 rounded-lg bg-emerald-500 text-white text-[10px] md:text-[11px] font-bold hover:bg-emerald-600 transition-all cursor-pointer active:scale-95 shadow-sm border-none">
                          <ArrowLeftRight className="w-4 h-4 md:w-3.5 md:h-3.5" /> Asignar
                        </button>
                      </div>
                    </div>
